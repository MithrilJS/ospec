...
// TODO: finish the local/globalAssertions suite
...
function registerLegacyMode(){
	lib.new = function(name) {
		var res = name ? _new(name) : _new()
		res.globalAssertions()
		return res
	}
}
function unregisterLegacyMode(){
	lib.new = _new
}
o.spec("legacy", function() {
	o.spec("o.localAssertions and o.globalAssertions", function() {
		o("global by default", function(done){
			var oo = lib.new()
			oo("global assertion", function(ooDone) {
				o(ooDone.spy).equals(undefined)
				o(ooDone.timeout).equals(undefined)
				o(ooDone.metadata).equals(undefined)
				ooDone()
			})
		...
		o("setting global at the root works", function(done){
			var oo = lib.new()
			oo.globalAssertions()
			oo("global assertion", function(ooDone) {
				o(ooDone.spy).equals(undefined)
		...
		o("setting local at the root works", done => {
			var oo = lib.new()
			oo.localAssertions()
			oo("local assertion", (ooo) => {
				o(typeof ooo.spy).equals("function")
				o(typeof ooo.timeout).equals("function")
				o(typeof ooo.metadata).equals("function")
				ooo(true).equals(true)
			})
			oo.run((results, stats) => {
		...
		o("with callbacks, localAssertions() and globalAssertions() can be nested and clean up after themselves", done => {
			var oo = lib.new()
			var SPY = o.spy()
			oo.globalAssertions() // future proofing the test suite for v5

			oo.localAssertions(() => {
				oo.globalAssertions(() => {
					oo.localAssertions(() => {
						oo("nested twice local assertions", (ooo) => {
							o(typeof ooo.spy).equals("function")
							o(typeof ooo.timeout).equals("function")
							o(typeof ooo.metadata).equals("function")
							SPY("...")
					...
					oo("nested global assertions", (ooDone) => {
						o(ooDone.spy).equals(undefined)
						o(ooDone.timeout).equals(undefined)
						o(ooDone.metadata).equals(undefined)
						SPY("..")
			...
			oo.run((results, stats) => {
				...
				o(SPY.calls.flatMap(call => call.args)).deepEquals(["...", "..", "."])
				done()
			})
		})
		o("calling localAssertions() or globalAssertions() in the wrong place throws SyntaxErrors", (done) => {
			var oo = lib.new()
			oo.spec("a spec", () => {
				oo("a test", () => {
					o(oo.localAssertions).throws(SyntaxError)
					o(oo.globalAssertions).throws(SyntaxError)	
				})
				oo.before(() => {
					o(oo.localAssertions).throws(SyntaxError)
					o(oo.globalAssertions).throws(SyntaxError)	
			...
			o("works", function(done) {
				...
				oo.run(function(results, stats) {
					try {
						...
						var only = stats.onlyCalledAt.map(function(line){return line.indexOf("test-api-legacy.js") !== -1})
					...
					if (err.stack) {
						...
						oo.run(function(results) {
							try{
								...
								o(oo.cleanStackTrace(results[0].task.error).indexOf("test-api-legacy.js:" + (line + 3) + ":")).notEquals(-1)
			...
			o.spec("stack trace cleaner", function() {
				o("handles line breaks", function() {
					...
					try {
						...
						o(trace.indexOf("test-api-legacy.js") !== -1).equals(true)
					}
				})
			})
...
o.spec("reporting", function() {
	o.before(registerLegacyMode)
	o.after(unregisterLegacyMode)
